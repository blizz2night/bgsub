...
def findContours(binaryImg, iteration = 2, kernel = np.ones((3, 3), np.uint8), pixels = 10):
...
#            self.bbox = []
    ...
    def getContourList(self):
        temp = np.array(self.contourList)*self.scale
        return temp.tolist()


    def getMaxRect(self):
        if len(self.contourRects)>0:
            return reduce(lambda x, y: x if x[2]*x[3] > y[2]*y[3] else y, self.contourRects)
        else:
            return []
...
while 1:
    ...
    if len(rects)>0:
        drawRectangles(frame,rects,(0,255,0))
        drawRectangles(frame,[id.getMaxRect()])
